...
def build_decoder(target_size=CFG.image_size):
    ...
    def decode_input(image_path, text):#,distance):
        image = decode_image(image_path)
        text = decode_text(text)
        #distance=decode_distance(distance)
        return {"images":image, "texts":text} #"distance":distance}
...
def build_dataset(
    ...
    slices = (image_paths, texts)#, distance)
...
train_ds = build_dataset(train_paths, train_texts,#train_distance,
...
valid_ds = build_dataset(valid_paths, valid_texts,#valid_distance,
                         batch_size=CFG.batch_size,
...
model.compile(optimizer="adam", loss=CLIPLoss())'''
...
tb_callback = tf.keras.callbacks.TensorBoard(log_dir='./datasets/logs', update_freq=1, histogram_freq=1)
...
history = model.fit(
        train_ds,
        epochs=CFG.epochs,
        callbacks=[lr_cb, ckpt_cb,tb_callback],
