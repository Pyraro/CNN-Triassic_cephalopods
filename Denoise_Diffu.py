...
list_ds = tf.data.Dataset.list_files(str("./Triassic_Cephalopod/Dataset_croped/Dataset_mixte+gbif_200/train/Clionites/*"), shuffle=False)
list_ds = list_ds.shuffle(10 * batch_size)
...
def decode_img(img):
  ...
  #return ops.clip(img / 255.0, 0.0, 1.0)
  return img
...
class DiffusionModel(keras.Model):
    ...
    def plot_images(self, epoch=None, logs=None, num_rows=3, num_cols=6):
        ...
        plt.tight_layout()
        #plt.show()
        plt.savefig('./checkpoints/fig_epoch_'+str(epoch)+".jpg")
        plt.close()
...
#checkpoint_path = "./checkpoints/diffusion_model.model.keras"
checkpoint_dir = './checkpoints'
checkpoint_prefix = os.path.join(checkpoint_dir, "ckpt_diffu")
checkpoint = tf.train.Checkpoint(model=model)
checkpoint_callback=tf.keras.callbacks.LambdaCallback( on_epoch_end= lambda epoch, logs: checkpoint.save(str(checkpoint_prefix)))
'''
